async function e({apiKey:e,model:t,content:n,systemInstruction:r,schema:i}){return(await(await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${t}:generateContent?key=${e}`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({contents:[{parts:[{text:n}]}],systemInstruction:{parts:[{text:r}]},generationConfig:{responseMimeType:`application/json`,responseSchema:i}})})).json()).candidates?.[0]?.content?.parts?.[0]?.text}(async()=>{let t=await BEM.getSetting(`apiKey`);if(!t){alert(`apiKey が設定されていません。`);return}let n=await e({apiKey:t,model:`gemini-3.1-flash-lite-preview`,content:`お世話になっております。
...
お忙しいところ恐縮ですが、納得のいく回答をお待ちしております。`,systemInstruction:`Summarize the message into exactly three concise bullet points in Japanese from a third-person perspective. Do not include any additional text.
...
3. The request, question, or desired resolution`,schema:{type:`array`,items:{type:`string`},minItems:3,maxItems:3}});console.log(n)})();
